...
const CreateProduct = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    let error = 0;
    // validate product data product name min 5 characters
    if (product.name.length < 1) {
      setError((prev) => ({ ...prev, name: true }));
      error++;
    }

    // validate product data product price must be greater than 10
    if (product.price < 1) {
      setError((prev) => ({ ...prev, price: true }));
      error++;
    }

    // validate product data product type must be at least 2 characters
          ...
          >
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                {error.price && (
                  <span data-testid="price-error" className="text-red-500">
                    price error
                  </span>
                )}
              </div>
              ...
              <div>
                ...
                />
                {error.type && (
                  <span data-testid="type-error" className="text-red-500">
                    type error
                  </span>
                )}
              </div>
